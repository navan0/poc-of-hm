...
NUM_COLS = [
    'duration', 'duration_ts', 'nb_frames', 'width', 'bit_rate', 'width',
    'height', 'coded_width', 'coded_height', 'start_time',
    'bits_per_raw_sample', 'nal_length_size']
...
def create_compressed_dataset(source_path, target_path):
    for i in tqdm(os.scandir(source_path)):
        ...
        crfs = [23, 40]
        new_bitrates = [
            int(old_bitrate * perc) for perc in bitrate_percentages]
...
def train_classifier(data=None, X=None, Y=None, path=None, save=False):
    ...
    x_train, x_test, y_train, y_test = train_test_split(
        copy, classes, test_size=0.2, shuffle=True)
